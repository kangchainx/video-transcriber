...
def download_media(video_url: str, workdir: Path, video_source: Optional[str] = None) -> Path:
    """
    下载音/视频到临时目录。
    ...
    if is_yt:
        return _download_youtube(video_url, workdir)
    return _download_http(video_url, workdir)


def _download_http(url: str, workdir: Path) -> Path:
    """普通 HTTP/HTTPS 下载。"""
